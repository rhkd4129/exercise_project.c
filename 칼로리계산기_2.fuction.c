...
void eat_list(void) {                    //먹은 음식 입력
	char food_number[10][10];
	char* p;
	p = food_number;
	int n = 0;
	int a = 1;
	...
	printf("먹은 음식들의 개수는?");
	scanf("%d", &n);
	printf("먹은 음식들을 입력하세요(음식 리스트와 정확히 입력해야됨)\n");

	for (int i = 0; i < n; i++,a++) {
		printf("%d번째 음식 입력",a);
		scanf("%s", *(food_number + i));
	...
	for (int i = 0; i < 10; i++) {
		for (int j = 0; j < 10; j++){
			...
			if (food_number[i][j] == '\0') {
				printf(" ");
			}
			else printf("%c", food_number[i][j]);
